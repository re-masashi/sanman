...
class Scanner(object):
    def __init__(self, patterns, ignore, input_):
    ...
    def token(self, i, restrict=0):
        ...
        if i < len(self.tokens):
            ...
            if restrict and self.restrictions[i]:
                ...
                return self.tokens[i]
        raise NoMoreTokens(i, len(
            self.tokens), self.tokens[i], restrict, self.restrictions[i], self.tokens)
    ...
    def scan(self, restrict):
        ...
        while 1:
            ...
            if best_pat == '(error)' and best_match < 0:
                msg = "Bad Token"
                if restrict:
                    msg = "Trying to find one of "+restrict+", "
...
class Parser(object):
    ...
    def _scan(self, type):
        ...
        if tok[2] != type:
            raise SyntaxError(
                tok[0], 'Trying to find %s (%s)' % (type, tok[0]))
...
def format_error(input_, err, scanner, filename=None):
    ...
    print(
         err.msg + "\033[1;31m " +" on line", 
        repr(line+1) + in_file+ " :" + 
        "\033[1;m",
        file=error_message,)
    # Now try printing part of the line
    text = input_[max(p-80, 0):p+80]
    p = p - max(p-80, 0)
    ...
    print('> ', text.replace('\t', ' ').encode(
        sys.getdefaultencoding()), file=error_message)
...
def wrap_error_reporter(parser, rule):
    try:
        ...
        input_ = parser._scanner.input
        try:
            error_msg = format_error(input_, e, parser._scanner, parser.filename)
        except ImportError:
            error_msg = 'Syntax Error %s on line\n' % (
                e.msg, 1 + input_[:e.pos].count("\n"))
