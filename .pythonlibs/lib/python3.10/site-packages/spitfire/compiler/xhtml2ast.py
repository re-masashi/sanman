...
class XHTML2AST(object):
    ...
    def build_ast(self, dom_node):
        ...
        if dom_node.attributes:
            ...
            #
            # fixme: content/replace are mutually exclusive, that should
            # generate an error the thing is, the way we process things is a
            # little complicated, so the order is actually different - we might
            # handle something like omit-tag early on, but really only apply
            # it's implications later on
            op_precedence = [
                'omit-tag',
                'define',
                'condition',
            ...
            # really) - also, some don't exactly make sense to be on the same
            # object as a repeat - for instance, repeat->replace, whereas
            # repeat->attributes makes more sense
            ...
            for op in op_precedence:
                ...
                if dom_node.hasAttribute(op_attr_name):  # in attr_name_list:
                    ...
                    # print ' '.join("op_handler:", op_handler,
                    #                dom_node.nodeName,
                    #                dom_node.attributes.keys(),
                    #                processed_any_op)

                    node_list.extend(getattr(self, op_handler)(dom_node,
                                                               op_attr_name))
    ...
    def make_tag_node(self, dom_node, close=False, attr_ast=None):
        ...
        if close:
            if self.has_child_stuff(dom_node):
                node_list.append(ast.TextNode('</%(node_name)s>' % vars()))
        else:
            attr_text = ' '.join(['%s="%s"' % (key, value)
                                  for key, value in list(dom_node.attributes.items())
            ...
            if self.has_child_stuff(dom_node):
                ...
                if attr_text:
                    ...
                    if attr_ast:
                        ...
                        node_list.append(ast.TextNode('<%(node_name)s />' %
                                                      vars()))
    ...
    def make_attr_node(self, attr):
        ...
        node_list.append(ast.TextNode(' %(new_attr_name)s="' % vars()))
