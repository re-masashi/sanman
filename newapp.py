...
env.compiled_templates["_1base"] = spitfire.compiler.util.load_template_file(
    "templates/_1base.spf"
)
...
async def index(request):
    return response.html(env.render_template("_1base", {}))
...
async def apisearch(request):
    ...
    bl = {"results": []}
    res = (
        requests.get(f"https://saavn.me/search/songs?query={q}&page={page}&limit=4")
        .json()
        .get("data")
    )
    respl = (
        requests.get(f"https://saavn.me/search/playlists?query={q}&page={page}&limit=4")
        .json()
        .get("data")
    )
    resal = (
        requests.get(f"https://saavn.me/search/albums?query={q}&page={page}&limit=4")
        .json()
        .get("data")
    )
    if res is None:
...
async def song_page(request, songid):
    ...
    data = requests.get("https://saavn.me/songs?id=" + songid).json()["data"][0]
...
async def artist_page(request, artistid):
    ...
    data = requests.get("https://saavn.me/artists?id=" + artistid).json()["data"]
